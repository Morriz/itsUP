...
Documentation lives in `docs/project/` as taxonomy-organized snippets (design,
spec, policy, procedure, adr). Each is code-grounded current truth; the index is
generated by `telec sync` at `docs/project/index.yaml`.

**Design** (`docs/project/design/`) — how the system works:

- [Network Segmentation](docs/project/design/network-segmentation.md) - the three network states, ingress/egress contract, lateral-movement threat model
- [Deployment & Orchestration](docs/project/design/deployment-orchestration.md) - zero-downtime rollout, change detection, stack ordering
- [Artifact Generation](docs/project/design/artifact-generation.md) - config → Traefik labels / dynamic routers, template+override merge
- [Security Architecture](docs/project/design/security-architecture.md) - defense-in-depth overview and threat model
- [Container Security Monitor](docs/project/design/container-security-monitor.md) - compromise detection via DNS-history correlation
- [Logging](docs/project/design/logging.md) - log files, formatting, the access.log → CrowdSec feed
- [itsUP CLI Distribution](docs/project/design/itsup-cli.md) - how `itsup` is packaged and invoked

**Spec** (`docs/project/spec/`) — the contracts:

- [Project Config](docs/project/spec/project-config.md) - `itsup-project.yml` ingress/egress/TLS contract
- [itsUP Config](docs/project/spec/itsup-config.md) - `itsup.yml` infrastructure config + traefik override merge
- [Secrets Management](docs/project/spec/secrets-management.md) - SOPS/age, per-context loading, `${VAR}` passthrough
- [Environment Variables](docs/project/spec/environment-variables.md) - the vars/secrets itsUP actually reads
- [API Surface](docs/project/spec/api-surface.md) - the apikey-guarded management REST API
- [Schema Migration](docs/project/spec/schema-migration.md) - config versioning and migration
- [CLI](docs/project/spec/cli.md) - non-obvious command semantics not in `--help`
- [Runtime Operations](docs/project/spec/runtime-operations.md) - scheduled operations + failure-diagnosis table

**Policy / Procedure / Decisions:**

- [OpenSnitch Database](docs/project/policy/opensnitch-database.md) - read-only forensic DB rule
- [Backup & Restore](docs/project/procedure/backup-and-restore.md) - back up to S3 and restore from loss
- [ADR-0001: single install root](docs/project/adr/0001-itsup-cli-single-root.md)
- [ADR-0002: block reverse-DNS egress](docs/project/adr/0002-block-reverse-dns-egress.md)

Agents discover these via `telec docs index --scope project` then `telec docs get <id>`.
...
[docs/project/design/network-segmentation.md](docs/project/design/network-segmentation.md)
