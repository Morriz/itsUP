...
- Untested: whether the agent's own admin token could add itself to a board it isn't a manager of, skipping the need for the human's token in step 3 too. `board-memberships/create.js`'s gate is being a project manager for that board's project, and it's unconfirmed whether the global admin role bypasses that specific gate (it does bypass the separate self-vs-others restriction). Don't assume either way — the human-token path in step 3 is the verified-safe one; only change it after testing.
