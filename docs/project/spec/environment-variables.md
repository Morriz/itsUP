...
| `LOG_LEVEL` | optional; default `INFO` | bridged to `ITSUP_LOG_LEVEL` by `lib/log_setup.py` for the supervised daemons, and at `bin/backup.py`, `bin/migrate_to_v2.py`, `bin/write_artifacts.py` for the one-shots | Default log level for the supervised daemons (API server, container security monitor) and for backup and the one-shot migration/artifact scripts. The supervised daemons configure a stdout handler through `lib/log_setup.py`, which does the bridging; the one-shots bridge it themselves before calling `configure_logging`. The CLI ignores it and maps `-v`/`-vv` to DEBUG/TRACE instead, setting `ITSUP_LOG_LEVEL` directly (`itsup/cli.py:46-62`). |
<!-- /planned-change:native-daemon-supervision -->
