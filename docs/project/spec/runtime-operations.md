...
| **reconcile workflow** (GitOps delivery + daily verification) | event (push to itsUP / projects / secrets / app repos) + daily schedule | On every config or code push, plus one scheduled no-op run per day | The shared `.github/workflows/reconcile.yml` first asserts its notification credentials (`TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` — missing credentials fail the run so an unnotifiable caller is a visible config error), then connects to the VPN, SSHes to the host, and runs the deploy **synchronously over the SSH session** under `set -e` (self-update for itsUP and for scheduled runs, `itsup apply <app>` for app images, pull+apply for config pushes), then asserts convergence against the applied-state receipt — `itsup verify <pushed-sha>` for itsUP/config pushes, `itsup verify --target <app>` for app deploys — and probes the API. Any failing step turns the run red; a failed run posts to the operator's Telegram. The daily scheduled run branches to self-update explicitly by event type and exercises the whole chain (VPN, SSH, deploy, API) so silent breakage surfaces as a red run within 24h. | A red workflow run in GitHub Actions and a Telegram message naming the failed run. | no (the next push or the next daily run retries; the red run is the signal) | The failed step's log in the GitHub Actions run; on the host `logs/applied-state.json` and `journalctl -u itsup-api.service`. | Fix the failing link (VPN credentials, SSH access, host state) and re-run the workflow from the Actions UI; `itsup verify <sha>` on the host reproduces the assertion. |
<!-- /planned:verifiable-deploy-chain -->
