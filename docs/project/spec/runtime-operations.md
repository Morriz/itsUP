...
| **DNS listener guard** | event (asserted by every DNS publish path) | Once per DNS publish — `itsup run` boot, `itsup dns up`, and any `itsup apply` that reaches the DNS stack | Root-owned oneshot unit `itsup-dns-guard.service` installs the `DOCKER-USER` rules confining the honeypot's published resolver on the proxynet gateway address to Docker bridge sources, matched pre-DNAT via conntrack original-destination. Every publish path calls a shared `require_dns_guard()` helper first, which re-asserts the rules through a **root-owned helper outside the repository tree** authorised by an exact-path sudoers entry — never a user-writable script, which would make repo write access a root-escalation path. The helper is idempotent: it reinstalls any missing rule and exits non-zero when it cannot, which aborts the publish. Rules carry an owned comment so `itsup monitor clear-iptables` cannot strip them. Linux only; on macOS the publish **refuses** rather than skipping, because publishing without containment would drop a mandatory security invariant. | DNS publish aborts with the guard named — no resolver is published — rather than a resolver appearing unprotected. | no (the publish refuses and stays refused until the guard can be asserted; nothing retries it silently) | `systemctl status itsup-dns-guard` and `journalctl -u itsup-dns-guard`; confirm the live rules with `sudo iptables -S DOCKER-USER \| grep itsup-dns-guard`; a unit reading `active` while that grep is empty is exactly the drift the assertion catches. | Re-assert by hand on the host: `sudo systemctl start itsup-dns-guard`, then re-run the failed publish. To remove entirely, `make uninstall-runtime` stops the unit and deletes its rules. |
<!-- /planned:dns-fallback-off-proxynet -->
