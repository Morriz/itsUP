...
| **pi-healthcheck** | timer | Every 5 min (`OnBootSec=5min`, `OnUnitActiveSec=5min`) | Checks mem/load/conntrack/disk and `docker ps`, and asserts the DNS listener guard on every run before any successful exit (see **DNS listener guard**): a repairable rule loss is repaired, and containment that cannot be restored withdraws the DNS stack and exits non-zero so the failure alert fires — the one action this timer takes outside the maintenance window that is not resource break-glass. The withdrawal is a deliberate fleet-wide container-DNS outage; recovery is to restore containment (fix the sudoers grant, helper or bridge discovery, then `sudo systemctl start itsup-dns-guard`) and republish with `itsup dns up`. Outside 02:30–03:30 the *resource* actions are **log-only** except break-glass thresholds (3-strike → restart docker + `itsup dns up`/`proxy up`). Inside the window it restarts docker+stacks on 1st strike (stamp `/run/itsup/pi-healthcheck.fail`), **reboots host** on 2nd. Strike state lives in the unit's own `RuntimeDirectory=itsup` (`/run/itsup`), created at unit start owned by its `User=` and held across runs by `RuntimeDirectoryPreserve=yes`; `/run` is tmpfs, so it clears on reboot. | Either silent (healthy) or, on a real degradation, an automatic stack restart / host reboot the operator did not initiate. | yes (this *is* the auto-recovery: restart stacks, then reboot if that fails) | `journalctl -u pi-healthcheck.service`; check `/run/itsup/pi-healthcheck.fail` and `/run/itsup/pi-healthcheck.strikes` for active strikes. | If false-tripping, clear strike state: `rm -f /run/itsup/pi-healthcheck.strikes /run/itsup/pi-healthcheck.fail`. To stop auto-action while debugging: `systemctl stop pi-healthcheck.timer`. |
<!-- /planned-change:dns-fallback-off-proxynet -->
...
| **DNS listener guard** | event (every DNS publish path) **plus** the 5-minute pi-healthcheck cycle | Once per DNS publish — `itsup run` boot, `itsup dns up`, any `itsup apply` reaching the DNS stack — and again every 5 minutes — so a rule loss can leave the listener **uncontained for up to five minutes** before the cycle detects it | Root-owned oneshot unit `itsup-dns-guard.service` installs the `DOCKER-USER` rules confining the honeypot's published resolver on the proxynet gateway address to discovered Docker bridge interfaces, matched pre-DNAT via conntrack original-destination. Every publish path calls a shared `require_dns_guard()` first, which runs `ensure` through a **root-owned helper outside the repository tree**, authorised by a sudoers entry naming the exact `ensure` command form — never the bare path, which would permit `remove`, and never a repo-writable script, which would make repo write access a root-escalation path. `ensure` is **self-healing**: it repairs missing rules and succeeds; it exits non-zero only when containment cannot be established (grant revoked, helper missing, `iptables` unavailable, bridge discovery ambiguous, non-Linux). Rules carry an owned comment so `itsup monitor clear-iptables` cannot strip them. Linux only; on macOS the publish **refuses** rather than skipping. | Before publication: the DNS publish aborts naming the guard and no resolver is published. After publication: pi-healthcheck **stops the DNS stack** (`itsup dns down`) when containment cannot be restored — a deliberate fleet-wide container-DNS outage, chosen over an uncontained resolver — and exits non-zero so the failure alert fires. | partial (missing rules are repaired automatically at publish and every 5 minutes; genuine incapacity is never self-healed — it blocks the publish, or withdraws an existing listener) | `systemctl status itsup-dns-guard` and `journalctl -u itsup-dns-guard`; confirm live rules with `sudo iptables -S DOCKER-USER \| grep itsup-dns-guard`; a unit reading `active` with that grep empty is exactly the drift the cycle repairs. For a withdrawal, `journalctl -u pi-healthcheck.service` names the failed `ensure`. | Restore containment, then republish: fix the cause (grant, helper, discovery), `sudo systemctl start itsup-dns-guard`, then `itsup dns up`. **Removal is a scoped inverse** — stop and disable the unit, run its `remove`, delete the helper and the sudoers drop-in — never `make uninstall-runtime`, which decommissions the whole host. |
<!-- /planned:dns-fallback-off-proxynet -->
