...
| **pi-healthcheck** | timer | Every 5 min (`OnBootSec=5min`, `OnUnitActiveSec=5min`) | Checks mem/load/conntrack/disk, `docker ps`, and probes the API (`GET localhost:8888/health`, bounded timeout) — a failed probe restarts `itsup-api.service` directly. Host vitals: outside 02:30–03:30 **log-only** except break-glass thresholds (3-strike → restart docker + `itsup dns up`/`proxy up`). Inside the window it restarts docker+stacks on 1st strike (stamp `/run/pi-healthcheck.fail`), **reboots host** on 2nd. | Either silent (healthy) or, on a real degradation, an automatic API-unit/stack restart or host reboot the operator did not initiate. | yes (this *is* the auto-recovery: restart the API unit / stacks, then reboot if that fails) | `journalctl -u pi-healthcheck.service`; check `/run/pi-healthcheck.fail` and `/run/pi-healthcheck.strikes` for active strikes. | If false-tripping, clear strike state: `rm -f /run/pi-healthcheck.strikes /run/pi-healthcheck.fail`. To stop auto-action while debugging: `systemctl stop pi-healthcheck.timer`. |
<!-- /planned-change:verifiable-deploy-chain -->
<!-- planned:verifiable-deploy-chain -->
| **API server** | event (boot; started by bringup where unsupervised) | Continuous host process on `:8888` | On Linux hosts the API runs as the supervised systemd unit `itsup-api.service` (`Restart=on-failure`): systemd starts it at boot and revives it on crash; `itsup run`/`down` delegate to `systemctl`. Where no unit is installed (macOS, dev) it is a plain process spawned by `bin/start-api.sh`. | Webhook deploys and `/health` probes fail; the scheduled reconcile workflow's API check turns the run red. | yes on Linux (`Restart=on-failure` revives a crashed process; pi-healthcheck restarts a hung-but-listening one); no where unsupervised | `systemctl status itsup-api.service` + `journalctl -u itsup-api.service`; `curl -sf localhost:8888/health`. | `systemctl restart itsup-api.service` (Linux); `bin/start-api.sh` elsewhere. |
| **reconcile workflow** (GitOps delivery + daily verification) | event (push to itsUP / projects / secrets / app repos) + daily schedule + manual dispatch (failure drill) | On every config or code push, plus one scheduled no-op run per day | The shared `.github/workflows/reconcile.yml` first asserts its notification credentials (`TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` — missing credentials fail the run so an unnotifiable caller is a visible config error), then connects to the VPN, SSHes to the host, and runs the deploy **synchronously over the SSH session** under `set -e` (self-update for itsUP and for scheduled runs, `itsup apply <app>` for app images, pull+apply for config pushes), then asserts convergence against the applied-state receipt — `itsup verify <pushed-sha>` for itsUP/config pushes, `itsup verify --target <app>` for app deploys — and probes the API. Any failing step turns the run red; a failed run posts to the operator's Telegram, and the notify step itself fails unless Telegram's response reports `ok: true` — a red run with an undelivered message is visible in the run log, never silent. A `workflow_dispatch` failure drill (`simulate_failure`) fails the run on purpose after the preflight, exercising the notification path end-to-end on demand. The daily scheduled run branches to self-update explicitly by event type and exercises the whole chain (VPN, SSH, deploy, API) so silent breakage surfaces as a red run within 24h. | A red workflow run in GitHub Actions and a Telegram message naming the failed run. | no (the next push or the next daily run retries; the red run is the signal) | The failed step's log in the GitHub Actions run; on the host `logs/applied-state.json` and `journalctl -u itsup-api.service`. | Fix the failing link (VPN credentials, SSH access, host state) and re-run the workflow from the Actions UI; `itsup verify <sha>` on the host reproduces the assertion; the failure drill re-tests notification delivery. |
<!-- /planned:verifiable-deploy-chain -->
