...
| **reconcile workflow** (GitOps delivery + daily verification) | event (push to itsUP / projects / secrets / app repos) + daily schedule | On every config or code push, plus one scheduled no-op run per day | The shared `.github/workflows/reconcile.yml` connects to the VPN, SSHes to the host, and runs the deploy **synchronously over the SSH session** (self-update for itsUP, `itsup apply <app>` for app images, pull+apply for config pushes), then asserts convergence with `itsup verify <pushed-sha>` against the applied-state receipt and probes the API. Any failing step turns the run red; a failed run posts to the operator's Telegram (bot credentials from repo secrets, skipped when unset). The daily scheduled run exercises the whole chain (VPN, SSH, deploy, API) so silent breakage surfaces as a red run within 24h. | A red workflow run in GitHub Actions and a Telegram message naming the failed run. | no (the next push or the next daily run retries; the red run is the signal) | The failed step's log in the GitHub Actions run; on the host `logs/applied-state.json` and `journalctl -u itsup-api.service`. | Fix the failing link (VPN credentials, SSH access, host state) and re-run the workflow from the Actions UI; `itsup verify <sha>` on the host reproduces the assertion. |
<!-- /planned:verifiable-deploy-chain -->
