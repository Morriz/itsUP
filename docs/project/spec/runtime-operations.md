...
| **pi-healthcheck** | timer | Every 5 min (`OnBootSec=5min`, `OnUnitActiveSec=5min`) | Checks mem/load/conntrack/disk and `docker ps`, and asserts the DNS listener guard on every run before any successful exit (see **DNS listener guard**): a repairable rule loss is repaired, and containment that cannot be restored exits the run non-zero so the failure alert fires, while the DNS stack keeps serving. The listener is never withdrawn — denying DNS to every container is a heavier cost than the residue an absent guard exposes on a non-LAN-routable address. Recovery is to restore containment (fix the sudoers grant, helper or bridge discovery, then `sudo systemctl start itsup-dns-guard`), which the next cycle confirms. Outside 02:30–03:30 the *resource* actions are **log-only** except break-glass thresholds (3-strike → restart docker + `itsup dns up`/`proxy up`). Inside the window it restarts docker+stacks on 1st strike (stamp `/run/itsup/pi-healthcheck.fail`), **reboots host** on 2nd. Strike state lives in the unit's own `RuntimeDirectory=itsup` (`/run/itsup`), created at unit start owned by its `User=` and held across runs by `RuntimeDirectoryPreserve=yes`; `/run` is tmpfs, so it clears on reboot. | Either silent (healthy) or, on a real degradation, an automatic stack restart / host reboot the operator did not initiate. | yes (this *is* the auto-recovery: restart stacks, then reboot if that fails) | `journalctl -u pi-healthcheck.service`; check `/run/itsup/pi-healthcheck.fail` and `/run/itsup/pi-healthcheck.strikes` for active strikes. | If false-tripping, clear strike state: `rm -f /run/itsup/pi-healthcheck.strikes /run/itsup/pi-healthcheck.fail`. To stop auto-action while debugging: `systemctl stop pi-healthcheck.timer`. |
<!-- /planned-change:dns-fallback-off-proxynet -->
...
| **failure alert** | event (a covered unit enters the `failed` state) | Once when a covered unit reaches `failed`; for units with `Restart=` (bringup, API, monitor), once per start-limit exhaustion rather than per individual failed attempt | Templated systemd unit run by every covered unit's `OnFailure=` hook. Composes a subject plus the failed unit's recent journal lines and pipes the result to the command in `alert.command` (`project/spec/itsup-config`). With the key unset it runs no command, records the suppressed alert in the journal, and exits 0. Runs as the itsUP user, never as root: journal reads come from `systemd-journal` group membership ensured at install, and SOPS decrypts the infra secrets the template's `${VAR}` placeholders resolve from using that user's own key. Operator-configured transport code executes without elevated privilege. The alert unit carries no `OnFailure=` of its own. A failing alert does not alert about itself. Linux/systemd only — launchd has no `OnFailure` equivalent, so macOS hosts have no failure hook. | The operator's configured channel stays silent while units fail. | no (a failed alert attempt is not retried; the composer's own failure is in its journal) | `journalctl -u 'itsup-alert@*'` for composer runs and their outcome; confirm the key with `grep -A2 '^alert:' projects/itsup.yml`; confirm the referenced secret is present in `secrets/itsup.enc.txt`. | Compose against a unit by hand on the host, as the itsUP user: `.venv/bin/python bin/alert.py <unit>`. |
<!-- planned:dns-fallback-off-proxynet -->
| **DNS listener guard** | event (every DNS publish path) **plus** the 5-minute pi-healthcheck cycle | Once per DNS publish — `itsup run` boot, `itsup dns up`, any `itsup apply` reaching the DNS stack — and again every 5 minutes — so a rule loss can leave the listener **unguarded for up to five minutes** before the cycle reports it | Root-owned oneshot unit `itsup-dns-guard.service` installs the `DOCKER-USER` rules confining the honeypot's published resolver on the proxynet gateway address to discovered Docker bridge interfaces, matched pre-DNAT via conntrack original-destination. Every publish path calls a shared `ensure_dns_guard()` first, which runs `ensure` through a **root-owned helper outside the repository tree**, authorised by a sudoers entry naming the exact `ensure` command form — never the bare path, which would permit `remove`, and never a repo-writable script, which would make repo write access a root-escalation path. `ensure` is **self-healing**: it repairs missing rules and succeeds; it reports failure only when containment cannot be established (grant revoked, helper missing, `iptables` unavailable, bridge discovery ambiguous, non-Linux). Rules carry an owned comment so `itsup monitor clear-iptables` cannot strip them. The guard is **defence-in-depth over a non-LAN-routable address**, so it gates nothing — an unestablished guard warns and the resolver publishes regardless. Both the guard and the published listener are **Linux-only, for separate reasons**: the guard because it is iptables-based, and the listener because binding a published port to the bridge gateway address requires the host to own that address, which a VM-backed Docker runtime's host does not (the bridges live inside the virtual machine). On such a host the daemon rejects the binding and the DNS stack does not start; that platform is not yet supported. | Resolution is unaffected on every path where the listener can bind. Before publication: the resolver is published and the run logs the guard warning. After publication: pi-healthcheck exits non-zero so the failure alert fires while the DNS stack keeps serving. The visible cost is an unguarded listener, never an outage. | partial (missing rules are repaired automatically at publish and every 5 minutes; genuine incapacity is never self-healed — it is alerted, and the listener keeps serving) | `systemctl status itsup-dns-guard` and `journalctl -u itsup-dns-guard`; confirm live rules with `sudo iptables -S DOCKER-USER \| grep itsup-dns-guard`; a unit reading `active` with that grep empty is exactly the drift the cycle repairs. `journalctl -u pi-healthcheck.service` names a failed `ensure`. | Restore containment: fix the cause (grant, helper, discovery) and `sudo systemctl start itsup-dns-guard`; the next cycle confirms it, and no republish is needed because the listener never stopped. **Removal is a scoped inverse** — stop and disable the unit, run its `remove`, delete the helper and the sudoers drop-in — never `make uninstall-runtime`, which decommissions the whole host. |
<!-- /planned:dns-fallback-off-proxynet -->
...
- **The supervision cutover record is host state, and its absence is not a
  ...
  record written: to bring the stack up, run `itsup run`, confirm it exited
  zero, then atomically write `complete`; to keep a host that is already as
  intended, atomically write `complete` and run nothing. A manual `itsup run`
  leaves the record absent, and an install with an absent record and installed
  definitions is ambiguous. An absent record never
  ...
  **Run ownership and retry differ by supervisor.** On Linux the bringup unit is
  `Type=oneshot` with `Restart=on-failure`, so a failed run is retried every 30
  seconds. Its start limit is bounded (`StartLimitIntervalSec=10min`,
  `StartLimitBurst=3`), so a persistent failure reaches `failed` and triggers the
  `OnFailure=` alert hook rather than retrying silently forever. On macOS the
  guardian runs under `set -euo pipefail` and its agent declares
