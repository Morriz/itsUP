...
- The bootstrap admin credentials (`DEFAULT_ADMIN_EMAIL`/`DEFAULT_ADMIN_USERNAME` + `DEFAULT_ADMIN_PASSWORD`) are known — they live in whatever secrets store the owning fleet uses (for an itsUP-operated instance, see `project/spec/secrets-management`).
