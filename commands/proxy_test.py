...
class TestProxy(unittest.TestCase):
    ...
    @patch("lib.deploy.deploy_proxy_stack")
    def test_proxy_up_regenerates_config(
        self, mock_deploy: Mock
    ) -> None:
        """Test that proxy up uses smart deployment."""
        # Reload module to pick up mocked deploy function
        import commands.proxy as proxy_module
        reload(proxy_module)

        result = self.runner.invoke(proxy_module.proxy, ["up"])
    ...
    def test_proxy_up_single_service(
        ...
        result = self.runner.invoke(proxy_module.proxy, ["up", "traefik"])
    ...
    @patch("commands.common.subprocess.run")
    @patch("lib.data.get_env_with_secrets")
    def test_proxy_down(self, mock_get_env: Mock, mock_subprocess: Mock) -> None:
        ...
        result = self.runner.invoke(proxy_module.proxy, ["down"])
    ...
    def test_proxy_restart(self, mock_get_env: Mock, mock_subprocess: Mock) -> None:
        ...
        result = self.runner.invoke(proxy_module.proxy, ["restart"])
    ...
    def test_proxy_logs(self, mock_get_env: Mock, mock_subprocess: Mock) -> None:
        ...
        result = self.runner.invoke(proxy_module.proxy, ["logs"])
